                ...
                if not up:
                    ...
                    if falhas > 0:
                        if not st.session_state.get("ignore_failed", True):
                            ...
                            with st.expander(f"⚠️ {len(failed_details)} Imagem(ns) com erro (clique para ver)", expanded=False):
                                ...
                                if failed_details:
                                    ...
                                    if st.button("📋 Copiar lista de falhas"):
    ...
    if st.session_state.preview_mode and st.session_state.pipeline and not st.session_state.quick_generate:
        with st.expander("2. Pré-visualização", expanded=True):
            ...
            if st.session_state.get("failed_details") and st.session_state.get("ignore_failed", True):
                ...
                # Opção para ver detalhes das falhas
                if st.checkbox("Mostrar detalhes das falhas", key="show_failed_details"):
                    with st.expander("📋 Detalhes das falhas", expanded=True):
                        if st.session_state.failed_details:
                            ...
                            if st.button("📋 Exportar lista de falhas (CSV)", key="export_failed"):

...
with st.expander("3. Gerar / Exportar", expanded=st.session_state.get("preview_mode", False)):
    if not (st.session_state.preview_mode and st.session_state.pipeline):
        ...
        # Placeholder para exibir a prévia só após carregamento
        preview_placeholder = st.empty()
        
        col_vis, col_gen, col_zip = st.columns([1, 1, 1])
        
        with col_vis:
            if st.button("🔎 Visualizar Prévia", use_container_width=True):
                # Inicia o carregamento com progresso
                progress_bar = st.progress(0)
                status_text = st.empty()
                
                # Calcula batches (parte demorada - divida em passos para progresso real)
                batches = build_batches_for_preview(items, resultados, cfg, st.session_state.excluded_urls)
                
                # Simula/calcula progresso (adapte para o seu loop real de geração)
                total_steps = len(batches) if batches else 1  # Ou use 100 para porcentagem genérica
                for step in range(total_steps + 1):
                    percentage = int((step / total_steps) * 100) if total_steps > 0 else 100
                    progress_bar.progress(percentage)
                    status_text.text(f"Carregando visualização: {percentage}%")
                    time.sleep(0.1)  # Simula delay; remova e atualize no seu loop real (ex.: por batch)
                
                # Após carregar, exibe a prévia no placeholder
                ...
                # Limpa o status após carregar
                status_text.empty()
        
        with col_gen:
            btn_generate = st.button("🧩 Gerar PDF", key="btn_generate", use_container_width=True)
            # Lógica de geração direta (sem esperar prévia) - igual ao original, mas mudei nome para PDF
            if btn_generate:
                try:
                    titulo = (st.session_state.output_filename or "Apresentacao").strip()
                    use_template = cfg.get("use_template", False)
                    ...
                    if use_template and template_bytes:
                        ...
                        # ... (o resto da lógica de geração sem template, igual ao original)
                        # (omitido por brevidade, mas mantenha do código original)
                    st.session_state.ppt_bytes = ppt_bytes  # Ou adapte para PDF real
                    st.session_state.generated = True
                    st.rerun()
                except Exception as e:
        ...
        with col_zip:
            # Botão de ZIP mantido, igual ao original
            if st.session_state.get("images_zip_bytes"):
                st.download_button(
                    "⬇️ Baixar Imagens (ZIP)",
                    data=st.session_state.images_zip_bytes,
        ...
        # Campo de nome do arquivo e download do PPT/PDF (igual ao original, mas renomeado)
        col1 = st.columns([3])[0]
        with col1:
            st.session_state.output_filename = st.text_input(
                "Nome base do arquivo (sem extensão)",
        ...
        if st.session_state.get("ppt_bytes"):
            st.download_button(
                "⬇️ Baixar PDF",
                data=st.session_state.ppt_bytes,
                file_name=f"{(st.session_state.output_filename or 'Apresentacao')}.pdf",  # Mudei extensão para .pdf
                mime="application/pdf",  # Mudei MIME para PDF (adapte se converter de verdade)
                use_container_width=True,
                key=f"download_{st.session_state.get('download_key', 0)}"
            )
